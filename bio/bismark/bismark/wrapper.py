...
with TemporaryDirectory() as tempdir:
    ...
    if single_end_mode:
        ...
        input_fq = os.path.join(tempdir, os.path.basename(snakemake.input.fq))
        cmdline_args.append("--se {input_fq:q}")
        mode_prefix = "se"
        if basename is None:
            basename = basename_without_ext(snakemake.input.fq)
    else:
        # for PE data, you should also spec ify read2 input by -I or --in2, and
        # specify read2 output by -O or --out2.
        input_fq1 = os.path.join(tempdir, os.path.basename(snakemake.input.fq_1))
        input_fq2 = os.path.join(tempdir, os.path.basename(snakemake.input.fq_2))

        cmdline_args.append("-1 {input_fq1:q} -2 {input_fq2:q}")
        mode_prefix = "pe"
    ...
    cmdline_args.append(f"--output_dir {tempdir} ")
    ...
    expected_2_actual_paths = [
        (
            ...
            os.path.join(
                tempdir, "{}{}.bam".format(basename, "" if single_end_mode else "_pe")
            ),
        ),
        (
